...
const QUOTATION_PASSWORD = "2026";
const QUOTATION_STORAGE_KEY = "sweetime-quotation-unlocked";

const heroChips = [
  "מיקוד בתוצאות אמיתיות",
  "מעל ‎₪10M+‎ בתקציבי פרסום",
  "שימוש נרחב ב-AI",
  "Meta / TikTok / WhatsApp / Loyalty"
] as const;
...
const proposalMeta = [
  { label: "עבור", value: "Sweetime נוף הגליל", forceLtr: false },
  { label: "הוכן ע״י", value: "Cee+", forceLtr: true },
...
function renderChipText(chip: string) {
  if (chip.includes("₪10M+")) {
    return (
      <>
        מעל <span dir="ltr">₪10M+</span> בתקציבי פרסום
      </>
    );
  }
...
function StripeLayer({
  ...
  return (
    <motion.div aria-hidden="true" className="pointer-events-none absolute inset-0" style={{ x, y, opacity }}>
      <div
        className="absolute inset-0"
        style={{
          backgroundImage: `repeating-linear-gradient(135deg, ${colorA} 0px, ${colorA} 22px, ${colorB} 22px, ${colorB} 54px)`
        }}
      />
    </motion.div>
  );
...
function ProposalSection({
  ...
  const stripeY = useTransform(scrollYProgress, [0, 1], ["-5%", "5%"]);
  const stripeX = useTransform(scrollYProgress, [0, 1], ["-2%", "2%"]);

  const tones = {
    white: {
      panel: "bg-white border-[#D9E5F1]",
      stripeA: "rgba(22,155,238,0.06)",
      stripeB: "rgba(242,56,147,0.02)",
      glowA: "rgba(22,155,238,0.14)",
      glowB: "rgba(242,56,147,0.10)"
    },
    blue: {
      panel: "bg-[linear-gradient(180deg,#F5FAFF_0%,#FFFFFF_100%)] border-[#D7E6F6]",
      stripeA: "rgba(28,162,235,0.10)",
      stripeB: "rgba(255,255,255,0.00)",
      glowA: "rgba(28,162,235,0.16)",
      glowB: "rgba(242,56,147,0.08)"
    },
    pink: {
      panel: "bg-[linear-gradient(180deg,#FFF6FB_0%,#FFFFFF_100%)] border-[#F0D8E5]",
      stripeA: "rgba(242,56,147,0.08)",
      stripeB: "rgba(22,155,238,0.03)",
      glowA: "rgba(242,56,147,0.16)",
      glowB: "rgba(22,155,238,0.10)"
    },
    blend: {
      panel: "bg-[linear-gradient(135deg,#FCFEFF_0%,#F2FAFF_52%,#FFF6FB_100%)] border-[#D9E5F1]",
      stripeA: "rgba(22,155,238,0.07)",
      stripeB: "rgba(242,56,147,0.06)",
      glowA: "rgba(22,155,238,0.14)",
      glowB: "rgba(242,56,147,0.14)"
    },
    cee: {
      panel: "bg-[#090B11] border-[#202531]",
      stripeA: "rgba(149,223,30,0.08)",
      stripeB: "rgba(255,255,255,0.01)",
      glowA: "rgba(149,223,30,0.18)",
      glowB: "rgba(149,223,30,0.08)"
    }
  } as const;
  ...
  return (
    <section ref={ref} id={id} className={cn("px-4 py-5 md:px-8 md:py-7", className)}>
      <div
        className={cn(
          "relative mx-auto max-w-[1240px] overflow-hidden rounded-[18px] border px-5 py-9 shadow-[0_24px_65px_rgba(16,32,68,0.08)] md:px-8 md:py-11",
          theme.panel,
          innerClassName
        )}
      >
        ...
        <motion.div
          aria-hidden="true"
          className="pointer-events-none absolute -left-20 top-6 h-48 w-48 blur-3xl"
          animate={{ x: [0, 18, 4], y: [0, 14, -8], opacity: [0.42, 0.62, 0.44] }}
          transition={{ duration: 10, repeat: Number.POSITIVE_INFINITY, repeatType: "mirror", ease: "easeInOut" }}
          style={{ background: `radial-gradient(circle, ${theme.glowA} 0%, transparent 72%)` }}
        />
        <motion.div
          aria-hidden="true"
          className="pointer-events-none absolute -bottom-20 right-0 h-56 w-56 blur-3xl"
          animate={{ x: [0, -18, 6], y: [0, -14, 8], opacity: [0.35, 0.56, 0.36] }}
          transition={{ duration: 12, repeat: Number.POSITIVE_INFINITY, repeatType: "mirror", ease: "easeInOut" }}
          style={{ background: `radial-gradient(circle, ${theme.glowB} 0%, transparent 74%)` }}
        />
...
function SectionHeading({
  ...
  return (
    <div className={cn("mx-auto max-w-4xl", align === "center" ? "text-center" : "text-right")}>
      ...
      <h2 className={cn("mt-3 text-[2rem] font-black leading-[1.08] md:text-[2.85rem]", dark ? "text-white" : "text-[#102044]")}>
        {title}
      </h2>
...
function QuotationButton({
  ...
  return (
    <a
      href={href}
      className={cn(
        "inline-flex items-center justify-center gap-2 rounded-[12px] px-5 py-3.5 text-sm font-bold transition duration-200 hover:-translate-y-0.5",
        variant === "primary" &&
          "bg-[#102044] text-white shadow-[0_16px_28px_rgba(16,32,68,0.18)] hover:bg-[#0B1A38]",
        variant === "secondary" &&
          "border border-[#B9D7EE] bg-[#16325F] text-white shadow-[0_10px_22px_rgba(16,32,68,0.07)] hover:bg-[#102044]",
        variant === "recommended" &&
          "bg-[#F23893] text-white shadow-[0_18px_30px_rgba(242,56,147,0.22)] hover:bg-[#D91D78]",
        variant === "cee" &&
...
function ProposalSummaryCard() {
  return (
    <div className="rounded-[24px] border border-[#D8E6F1] bg-white px-5 py-5 shadow-[0_18px_45px_rgba(16,32,68,0.08)] md:px-6 md:py-6">
      <div className="flex items-start justify-between gap-4">
        <div className="space-y-2">
          <p className="text-sm font-bold tracking-[0.16em] text-[#169BEE]">פרטי ההצעה</p>
          <h3 className="text-[1.4rem] font-black text-[#102044]">Sweetime נוף הגליל</h3>
        </div>
        <Image
          ...
          width={184}
          height={56}
          className="h-auto w-[96px] opacity-95"
        />
      </div>

      <div className="mt-5 divide-y divide-[#E6EEF5]">
        {proposalMeta.map((item) => (
          <div key={item.label} className="flex items-center justify-between gap-6 py-3 text-sm">
            <span className="text-[#6A7890]">{item.label}</span>
            <span className="font-bold text-[#102044]" dir={item.forceLtr ? "ltr" : undefined}>
              {item.value}
            </span>
          </div>
        ))}
...
function PricingCard({ plan, index }: { plan: (typeof quotationData.plans)[number]; index: number }) {
  ...
  return (
    <motion.article
      ...
      className={cn(
        "relative flex h-full flex-col rounded-[18px] border bg-white p-5 shadow-[0_18px_42px_rgba(16,32,68,0.08)]",
        plan.recommended ? "border-[#F4B4D5] ring-1 ring-[#F4B4D5]/55" : "border-[#D8E4EE]"
      )}
    >
      <div
        className={cn(
          "absolute inset-x-0 top-0 h-1.5 rounded-t-[18px]",
          plan.recommended
      ...
      <div className="mt-2 flex items-start justify-between gap-4">
        <div className="space-y-2">
          <div
            className={cn(
              "inline-flex rounded-[999px] px-3 py-1 text-xs font-bold",
              plan.recommended ? "bg-[#FFF0F8] text-[#B51667]" : "bg-[#EEF7FF] text-[#0F5E9D]"
            )}
          >
            {plan.badge ?? plan.label}
          </div>
          <h3 className="text-center text-[1.82rem] font-black leading-[1.02] text-[#102044]">{plan.name}</h3>
          {plan.badge ? <p className="text-sm font-bold text-[#F23893]">{plan.label}</p> : null}
        </div>

        <div
          className={cn(
            "flex h-11 w-11 items-center justify-center rounded-[14px] text-base font-black text-white",
      ...
      <div className="mt-5 grid gap-3">
        <div className="rounded-[14px] border border-[#D8E4EE] bg-[#F7FBFF] px-4 py-3">
        ...
        <div className="rounded-[14px] border border-[#F8D0E3] bg-[#FFF7FB] px-4 py-3">
        ...
        {totalInvestment ? (
          <div className="rounded-[14px] bg-[#102044] px-4 py-3 text-white">
      ...
      {plan.note ? (
        <div className="mt-5 rounded-[14px] border border-[#D8E4EE] bg-[#F7FBFF] px-4 py-3 text-sm leading-7 text-[#425168]">
          {plan.note}
        </div>
      ) : null}
      ...
      {plan.scalingNote ? (
        <div
          className={cn(
            "mt-5 rounded-[14px] px-4 py-3 text-sm leading-7",
...
function RecommendationButton() {
  return (
    <QuotationButton
      ...
      variant="cee"
      className="w-full max-w-[360px] items-center justify-between px-5 py-4"
    >
...
function QuotationUnlockOverlay() {
  ...
  useEffect(() => {
    ...
    const saved = window.sessionStorage.getItem(QUOTATION_STORAGE_KEY);
    if (saved === QUOTATION_PASSWORD) {
      setUnlocked(true);
      return;
    }
  ...
  const submit = () => {
    if (password === QUOTATION_PASSWORD) {
      window.sessionStorage.setItem(QUOTATION_STORAGE_KEY, QUOTATION_PASSWORD);
      setUnlocked(true);
      setVisible(false);
      setError("");
      return;
  ...
  return (
    <>
      <motion.div
        aria-hidden="true"
        className="fixed inset-0 z-40 bg-white/12 backdrop-blur-[2px]"
        animate={{
          opacity: locked ? 1 : 0,
          backdropFilter: locked ? "blur(18px)" : "blur(0px)"
        }}
      ...
      <motion.div
        ...
        animate={{ opacity: locked && visible ? 1 : 0 }}
        transition={{ duration: 0.35, ease: "easeOut" }}
        style={{ pointerEvents: locked ? "auto" : "none" }}
      >
        <motion.div
          initial={{ y: 28, scale: 0.98 }}
          animate={{ y: locked && visible ? 0 : 18, scale: locked && visible ? 1 : 0.98 }}
          transition={{ duration: 0.35, ease: [0.22, 1, 0.36, 1] }}
          className="w-full max-w-[430px] rounded-[24px] border border-[#D9E5F1] bg-white p-6 text-center shadow-[0_30px_90px_rgba(16,32,68,0.16)]"
        >
          ...
          <p className="mt-6 text-[1.55rem] font-black text-[#102044]">גישה להצעת המחיר</p>
          <p className="mt-3 text-sm leading-7 text-[#4D5D74]">
            כדי לפתוח את ההצעה, יש להזין את הסיסמה שנמסרה על ידי נציג <span dir="ltr">Cee+</span>.
          </p>
          ...
          <div className="mt-6 space-y-3">
            <input
              ...
              autoFocus
              className="w-full rounded-[14px] border border-[#D9E5F1] bg-[#F7FBFF] px-4 py-3 text-center text-base font-bold text-[#102044] outline-none transition focus:border-[#169BEE] focus:bg-white"
              placeholder="הזנת סיסמה"
              aria-label="סיסמת גישה"
            />
            {error ? <p className="text-sm font-bold text-[#D91D78]">{error}</p> : null}
            <button
              type="button"
              onClick={submit}
              className="inline-flex w-full items-center justify-center rounded-[14px] bg-[#102044] px-4 py-3 text-sm font-bold text-white shadow-[0_16px_28px_rgba(16,32,68,0.16)] transition hover:bg-[#0B1A38]"
            >
              פתיחת ההצעה
            </button>
          </div>
...
export function SweetimeQuotationPage() {
  const recommendedPlan = useMemo(
    () => quotationData.plans.find((plan) => plan.recommended) ?? quotationData.plans[2],
    []
  );
  ...
  return (
    ...
    >
      <QuotationUnlockOverlay />

      <ProposalSection tone="blend" className="pt-6">
        <div className="grid gap-8 lg:grid-cols-[0.53fr_0.47fr] lg:items-center">
          <SectionReveal>
            <div className="space-y-6">
              <div className="space-y-4">
                <div className="inline-flex rounded-[999px] border border-[#F8D0E3] bg-[#FFF3FA] px-4 py-2 text-sm font-bold text-[#C41F72]">
                  תוכנית צמיחה שיווקית ופרפורמנס עבור Sweetime
                </div>
                <div className="flex items-center gap-3">
                  ...
                  />
                  <div className="h-10 w-px bg-[#D8E4EE]" />
                  <Image
                    src="/quotation/sweetime/cee-wordmark.webp"
                    alt="לוגו Cee+"
                    width={184}
                ...
                <h1 className="text-[2.65rem] font-black leading-[1.02] text-[#102044] md:text-[4.25rem]">
                  {quotationData.hero.headline}
                </h1>
                <p className="max-w-2xl text-[1.18rem] font-bold leading-9 text-[#D21C77] md:text-[1.46rem]">
                  {quotationData.hero.subheadline}
                </p>
                <p className="max-w-2xl text-base leading-8 text-[#425168] md:text-lg">
                  {quotationData.hero.supportingText.replace("ההצעה הזו נבנתה", "ההצעה נבנתה")}
                </p>
              </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {heroChips.map((chip) => (
                  <div
                    key={chip}
                    className="flex items-center gap-3 rounded-[14px] border border-[#D8E4EE] bg-white px-4 py-3 text-sm font-bold text-[#102044] shadow-[0_10px_20px_rgba(16,32,68,0.04)]"
                  >
                    <span className="flex h-9 w-9 items-center justify-center rounded-[12px] bg-[#EAF7FF] text-[#169BEE]">
          ...
          <SectionReveal>
            <div className="overflow-hidden rounded-[28px] border border-[#D8E4EE] bg-white p-3 shadow-[0_24px_60px_rgba(16,32,68,0.10)]">
              <Image
                src="/quotation/sweetime/sweethero.webp"
                alt="מערכת הצמיחה של Sweetime"
                width={1280}
                height={1280}
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {positioningCards.map((card) => (
            <motion.article
              key={card.title}
              whileHover={{ y: -4 }}
              className="rounded-[18px] border border-[#D8E4EE] bg-[linear-gradient(180deg,#ffffff,#f8fbff)] p-5 shadow-[0_14px_32px_rgba(16,32,68,0.05)]"
            >
              <div className="flex h-12 w-12 items-center justify-center rounded-[14px] bg-[#EEF7FF] text-[#169BEE]">
      ...
      <ProposalSection id="growth-engine" tone="blue">
        ...
        <div className="mt-8 grid gap-3 lg:grid-cols-7">
          {quotationData.growthEngine.flow.map((step, index) => (
            <div key={step} className="relative rounded-[18px] border border-[#D2E6F5] bg-white px-4 py-4 text-center shadow-[0_12px_26px_rgba(16,32,68,0.04)]">
              <div className="mx-auto flex h-10 w-10 items-center justify-center rounded-[12px] bg-[#169BEE] text-sm font-black text-white">
                {index + 1}
              </div>
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="rounded-[20px] border border-[#D8E4EE] bg-white p-4 shadow-[0_18px_40px_rgba(16,32,68,0.07)]">
            <div className="mx-auto max-w-[340px] rounded-[20px] bg-[#0F2042] p-3 shadow-[0_18px_42px_rgba(16,32,68,0.16)]">
              <div className="overflow-hidden rounded-[16px] border border-white/10 bg-white">
                <Image
            ...
            <div className="mt-4 rounded-[16px] border border-[#F8D0E3] bg-[#FFF5FA] px-4 py-3 text-sm leading-7 text-[#A01F63]">
          ...
          <div className="rounded-[20px] border border-[#D8E4EE] bg-white p-5 shadow-[0_18px_40px_rgba(16,32,68,0.07)]">
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              {["10% הנחה", "קינוח מתנה", "הטבת יום הולדת", "מבצע חברי מועדון"].map((reward) => (
                <div
                  key={reward}
                  className="rounded-[14px] border border-[#F8D0E3] bg-[#FFF7FB] px-4 py-3 text-sm font-bold text-[#B11F67]"
                >
                  {reward}
                </div>
              ))}
            ...
            <div className="mt-5 rounded-[16px] border border-[#D8E4EE] bg-[#F7FBFF] px-4 py-4">
        ...
        <div className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
          <div className="space-y-5">
            <SectionHeading
              ...
              align="right"
            />
            <div className="rounded-[16px] border border-[#F8D0E3] bg-white px-5 py-4 text-sm leading-7 text-[#4D5D74] shadow-[0_12px_28px_rgba(242,56,147,0.06)]">
            ...
            <div className="rounded-[16px] bg-[#102044] px-5 py-4 text-white">
          ...
          <div className="space-y-4">
            <div className="rounded-[20px] border border-[#D8E4EE] bg-white p-4 shadow-[0_18px_40px_rgba(16,32,68,0.07)]">
              <div className="overflow-hidden rounded-[16px] border border-[#D8E4EE] bg-[#F7FBFF] p-3">
                <Image
              ...
              <div className="mt-4 rounded-[16px] border border-[#D8E4EE] bg-[#F7FBFF] px-4 py-3 text-sm leading-7 text-[#4D5D74]">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              {quotationData.orderingWebsite.products.map((product, index) => (
                <div
                  key={product}
                  className={cn(
                    "rounded-[16px] border bg-white px-4 py-4 shadow-[0_10px_24px_rgba(16,32,68,0.04)]",
      ...
      <ProposalSection tone="white">
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {includedServices.map((item) => (
            <div key={item.title} className="rounded-[18px] border border-[#D8E4EE] bg-white p-5 shadow-[0_14px_32px_rgba(16,32,68,0.05)]">
              <div className="flex h-11 w-11 items-center justify-center rounded-[14px] bg-[#EEF7FF] text-[#169BEE]">
      ...
      <ProposalSection id="why-cee" tone="cee" innerClassName="shadow-[0_24px_60px_rgba(8,10,14,0.34)]">
        <div className="grid gap-8 lg:grid-cols-[0.84fr_1.16fr] lg:items-start">
          <div className="space-y-5">
            ...
            <div className="rounded-[20px] border border-[#95DF1E]/20 bg-[#0F131C] px-6 py-8 text-white shadow-[0_18px_40px_rgba(149,223,30,0.10)]">
              <div className="flex items-start justify-between gap-4">
                <div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {trustCards.map((card) => (
              <div key={card.title} className="rounded-[18px] border border-[#202531] bg-[#0F131C] p-5 shadow-[0_14px_32px_rgba(0,0,0,0.22)]">
                <div className="flex h-11 w-11 items-center justify-center rounded-[14px] bg-[#95DF1E]/12 text-[#95DF1E]">
                  <card.icon className="h-5 w-5" />
                </div>
      ...
      <ProposalSection id="recommendation" tone="blend" className="pb-10">
        <div className="grid gap-8 lg:grid-cols-[1.06fr_0.94fr] lg:items-center">
          <div className="space-y-5">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-[16px] border border-[#D8E4EE] bg-white px-4 py-4 shadow-[0_10px_22px_rgba(16,32,68,0.04)]">
              ...
              <div className="rounded-[16px] border border-[#F8D0E3] bg-[#FFF6FB] px-4 py-4 shadow-[0_10px_22px_rgba(242,56,147,0.05)]">
              ...
              {recommendedTotal ? (
                <div className="rounded-[16px] bg-[#102044] px-4 py-4 text-white shadow-[0_14px_32px_rgba(16,32,68,0.12)] sm:col-span-2">
            ...
            <div className="grid gap-3">
              {finalHighlights.map((item) => (
                <div
                  key={item}
                  className="flex items-start gap-3 rounded-[16px] border border-[#D8E4EE] bg-white px-4 py-3 text-sm leading-7 text-[#102044] shadow-[0_10px_22px_rgba(16,32,68,0.04)]"
                >
                  <CheckCircle2 className="mt-1 h-4 w-4 shrink-0 text-[#169BEE]" />
                  <span>{item}</span>
                </div>
          ...
          <div className="rounded-[20px] border border-[#D8E4EE] bg-white p-5 shadow-[0_18px_40px_rgba(16,32,68,0.07)]">
            ...
            <div className="mt-5 space-y-3">
              {quotationData.finalRecommendation.items.map((item) => (
                <div key={item} className="flex items-start gap-3 rounded-[14px] bg-[#F7FBFF] px-4 py-3 text-sm leading-7 text-[#102044]">
                  <CheckCircle2 className="mt-1 h-4 w-4 shrink-0 text-[#169BEE]" />
                  {item}
                </div>
              ))}
            ...
            <div className="mt-5 rounded-[16px] border border-[#F8D0E3] bg-[linear-gradient(135deg,#FFF4FB,#F7FBFF)] px-4 py-4 text-sm leading-7 text-[#425168]">
            ...
            <div className="mt-5 rounded-[16px] bg-[#102044] px-4 py-4 text-sm leading-7 text-white">
